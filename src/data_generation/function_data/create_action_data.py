...
def get_scenes(scenes, scanName):
    ...
    for scene in scenes:
        ...
        if startIndex == len(actions) - 1:
            raise Exception("path is too short")
        traj_feats[scene] = {}
        total = 0
        forward = 0
        stop = 0
        for n1 in range(startIndex, len(actions), 10):
            for n2 in range(n1 + 1, len(actions), 2):
...
if __name__ == "__main__":
    ...
    args.dataset = "gibson"
    base_dir = f"/home/blackfoot/codes/NRNSD/data/topo_nav/{args.dataset}/"
    args.data_splits = f"/home/blackfoot/codes/NRNSD/data/data_splits/{args.dataset}/"
